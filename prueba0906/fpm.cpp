...
// FPMStatus FPM::uploadImage(Stream &imageStream)
// {
//     buffer[0] = FPM_GETIMAGE;
//     writePacket(FPM_COMMANDPACKET, buffer, 1);
...
//     FPMStatus status = readAckGetResponse(&confirmCode, &readLen);
//     if (status != FPMStatus::OK || confirmCode != FPMStatus::OK) {
//         return status != FPMStatus::OK ? status : confirmCode;
//     }
    
//     // Leer datos de la imagen
//     uint16_t bytesRead = 0;
//     uint8_t data[32];
//     while (bytesRead < 19200) { // Adjust according to the image size
//         if (port->available() >= 32) {
//             port->readBytes(data, 32);
//             imageStream.write(data, 32);
//             bytesRead += 32;
//         }
//     }
...
// FPMStatus FPM::uploadImage(byte* buffer, uint16_t size) {
//     if (size > FPM_MAX_PACKET_LEN) {
//         return FPMStatus::READ_ERROR; // Verifica si el tamaño de la imagen es válido
//     }
...
//     writePacket(FPM_COMMANDPACKET, buffer, 3);
//     FPMStatus status = getResponse();

//     // Verifica la respuesta y actúa en consecuencia
//     if (status == FPMStatus::ACK) {
//         return FPMStatus::UPLOAD_SUCCESS;
//     } else {
//         // Hubo un error al subir la imagen
//         return status; // Devuelve el código de error específico
//     }}
...
FPMStatus FPM::uploadImage(uint8_t* buffer, size_t bufferSize) {
    // Establece el código de instrucción para la carga de imágenes
    buffer[0] = FPM_IMGUPLOAD;

    // Copia los datos del búfer al búfer interno del sensor
    memcpy(&buffer[1], buffer, bufferSize);
    
    // Envía el comando al sensor y espera la respuesta
    return writeCommandGetResponse(bufferSize + 1); // Añadimos 1 para incluir el byte de instrucción
}
